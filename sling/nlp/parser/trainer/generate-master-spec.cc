...
DEFINE_int32(word_embeddings_dim, 2, "Word embeddings dimensionality.");
...
DEFINE_bool(oov_lstm_features, false,
...
void OutputMasterSpec(Artifacts *artifacts) {
  ...
  SetParam(lr_lstm->mutable_network_unit(), "hidden_layer_sizes", "8");
  ...
  if (FLAGS_oov_lstm_features) {
    AddFixedFeature(
        lr_lstm, "suffix", "suffix", 2, artifacts->num_suffixes,
        kMaxSuffixLength);
  ...
  *rl_lstm = *lr_lstm;
  ...
  SetParam(ff->mutable_network_unit(), "hidden_layer_sizes", "8");
  ...
  if (fr > 0) {
    AddFixedFeature(ff, "in-roles", "in-roles", arg, 2, fr, kMaxRoleIds);
    AddFixedFeature(ff, "out-roles", "out-roles", arg, 2, fr, kMaxRoleIds);
    AddFixedFeature(
        ff, "labeled-roles", "labeled-roles", arg, 2, f2 * roles, kMaxRoleIds);
    AddFixedFeature(
        ff, "unlabeled-roles", "unlabeled-roles", arg, 2, f2, kMaxRoleIds);
  }

  AddLinkedFeature(ff, "frame-creation-steps", "frame-creation", 2, 2, "ff");
  AddLinkedFeature(ff, "frame-focus-steps", "frame-focus", 2, 2, "ff");
  AddLinkedFeature(ff, "frame-end-lr", "frame-end", 2, 2, "lr_lstm");
  AddLinkedFeature(
      ff, "frame-end-rl", "frame-end", 2, 2, "rl_lstm", "reverse-token");
  AddLinkedFeature(ff, "history", "history", 2, 2, "ff", "history");
  AddLinkedFeature(ff, "lr", "focus", 1, 2, "lr_lstm");
  AddLinkedFeature(ff, "rl", "focus", 1, 2, "rl_lstm", "reverse-token");
